...
public:
	void FillAttributes(AttrContext &context, symbol_ptr &tree_node) override {
        try{
            ...
            if(context.currentNameSpace!=context.globalNameSpace){
                emitError(context,tree_node,"Can not define function in function!");
            }
...
//        context.currentNameSpace->remove(decl->identifier);
            if(func_item->func->idDefined()){
                emitError(context,tree_node, "Function redefined!");
                return;
            }
